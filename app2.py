# smart_study_buddy_v3.py
"""
Smart Study Buddy v3.0
Includes:
- Chat memory (follow-ups)
- Page preview thumbnails (requires PyMuPDF / fitz)
- Quiz mode (MCQs)
- Session analytics (count, avg response time, top topics)
- Long response collapsing
...
# PAGE CONFIG
# -------------------------
st.set_page_config(page_title="Smart Study Buddy v3.0", layout="wide")

# Minimal CSS polish (no emojis requested earlier; this keeps it clean)
st.markdown("""
<style>
...
def create_page_thumbnails(pdf_path: str, max_pages=8):
    ...
    try:
        ...
        # fail silently and return empty dict
        return {}
    return page_images
...
class OpenAIFallbackModel:
    """
    Lightweight wrapper to provide a generate_text(prompt) style interface
    that ChatHandler can use if it expects a generic model.
    Uses openai.ChatCompletion or completions depending on availability.
    """
...
def initialize_model_with_fallback():
    """
    Try to initialize Gemini; on failure, try OpenAI fallback if OPENAI_API_KEY present.
    Returns a model object compatible with ChatHandler (best-effort).
    """
    gem_key = os.getenv('GEMINI_API_KEY')
    if gem_key:
        try:
            genai.configure(api_key=gem_key)
            # using the same call you had originally
            return genai.GenerativeModel('gemini-2.5-flash'), 'gemini'
        except Exception as e:
            st.warning(f"Gemini init failed, attempting OpenAI fallback: {str(e)}")
    ...
    if openai_key and HAS_OPENAI:
        ...
        st.error("No GEMINI_API_KEY set and OpenAI fallback not available (install openai and set OPENAI_API_KEY).")
        st.stop()
...
def process_documents(uploaded_files):
    ...
    for idx, f in enumerate(uploaded_files):
        ...
        try:
            ...
            if not docinfo:
                ...
                continue
            # attach document name to chunk metadata
    ...
    # If ChatHandler expects a genai model object, it will use it directly.
    # If we passed OpenAIFallbackModel, ChatHandler may need to call generate_text.
    ...
    # set insights to first doc if single
...
def build_memory_context():
    """
    Build a short text context from session memory buffer (deque)
    expecting pattern: [user, assistant, user, assistant, ...], max length 6 (3 exchanges)
    Returns combined short history string to prepend to queries.
    """
...
def add_query_stats(duration, user_text):
    ...
    # record keywords from user text
    # we'll store top topics aggregated on demand
...
def display_analytics():
    ...
    # plot response times distribution
...
def generate_quiz(num_questions=5):
    """
    Use chat_handler to generate MCQs from the document by prompting.
    We call chat_handler.get_response with a quiz prompt.
    """
    ...
    quiz = []
    try:
        # naive attempt to find JSON block
        json_text = response[response.find('{'):response.rfind('}')+1]
        data = json.loads(json_text)
        for k, v in data.items():
            quiz.append(v)
    except Exception:
        # fallback: show response raw and rely on user to parse
        st.warning("Could not parse quiz JSON; showing raw output.")
        st.write(response)
        return

    # Display quiz with "Show Answers" toggle
...
def sidebar_controls():
    with st.sidebar:
        ...
        st.markdown("---")
        st.markdown("### About")
        st.markdown("Smart Study Buddy v3.0 — built for hackathon. Features: multi-document RAG, voice, quizzes, analytics.")
...
def main():
    sidebar_controls()
    st.title("Smart Study Buddy v3.0")
    st.markdown("Multi-document AI assistant with memory, page previews, quizzes, and analytics.")
    ...
    if not st.session_state.vector_store:
        ...
        if st.session_state.insights:
            with st.expander("Document Insights", expanded=False):
                display_analytics()
                # show first doc insights if single
                if len(st.session_state.documents) == 1:
        ...
        st.markdown("---")
        # chat area and page preview column
        left, right = st.columns([3,1])
        with right:
            st.markdown("### Page Previews")
            # show thumbnails for each document
            for name, info in st.session_state.documents.items():
                st.markdown(f"**{name}**")
        ...
        with left:
            ...
            if user_input and not st.session_state.processing:
                ...
                # build final prompt by prepending memory
                final_prompt = memory_ctx + "\nUser: " + user_input
                # pick adaptive k (reuse your earlier helper if present)
                ...
                try:
                    ...
                    # update memory buffer: push user and assistant content
